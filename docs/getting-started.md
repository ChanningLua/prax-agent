...
| 装了但说 `ModuleNotFoundError: prax` | npm 装的包不在 PYTHONPATH。直接退回用 `pip install prax-agent` |
...
**全是 ✗？** 回头检查 Step 2 的 `export` 有没有跑成功。`echo $ZHIPU_API_KEY` 应该回显出你的 key。
...
| 输出是空行然后就停了 | 模型对接层出了问题 | `prax doctor all` 看详细诊断 |
