...
def _models_config() -> dict:
    return {
        ...
        "upgrade_chain": ["gpt-5.5", "claude-sonnet-4-8"],
        "providers": {
            "zhipu": {
...
def test_parse_command_tokens_and_slash():
    ...
    slash = parse_slash_command("/model claude-sonnet-4-8")

    assert command == ParsedCommand(name="status", args=[])
    assert slash == ParsedCommand(name="model", args=["claude-sonnet-4-8"])
...
def test_model_catalog_marks_availability(monkeypatch):
    ...
    assert by_name["gpt-5.5"].matches("gpt") is True
    assert by_name["claude-sonnet-4-8"].available is False
    assert by_name["codex"].available is False
    assert get_first_available_model(["claude-sonnet-4-8", "gpt-5.5"], _models_config()).name == "gpt-5.5"
...
def test_run_command_session_and_cost(tmp_path):
    ...
    session = SessionData(
        ...
        metadata={
            ...
            "upgrade_history": [{"from": "gpt-5.5", "to": "claude-sonnet-4-8", "reason": "tool_error"}],
            "last_run": {
                "model": "gpt-5.5",
                "executor": "direct-api",
                "runtime_path": "native",
...
def test_run_command_init_models_set_default(tmp_path, monkeypatch):
    ...
    assert "default_model: claude-sonnet-4-8" in local_models
...
def test_run_command_model_updates_session(tmp_path):
    ...
    result = run_command(ParsedCommand(name="model", args=["claude-sonnet-4-8"]), ctx)
    saved = store.load("session_demo")

    assert json.loads(result.text)["preferred_model"] == "claude-sonnet-4-8"
    assert saved is not None
    assert saved.metadata == {"preferred_model": "claude-sonnet-4-8"}
