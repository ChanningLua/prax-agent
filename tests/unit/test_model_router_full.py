...
def test_default_routing_rules_present():
    router = ModelRouter()
    assert router._routing_rules["default"] == "claude-opus-4-8"
    assert router._routing_rules["chinese_content"] == "glm-4-flash"
    assert router._routing_rules["debugging"] == "gpt-5.5"
    assert router._routing_rules["architecture"] == "claude-opus-4-8"
...
def test_route_force_model_overrides_classification():
    ...
    model = router.route("用中文写代码", context={"force_model": "gpt-5.5"})
    assert model == "gpt-5.5"
...
def test_get_fallback_chain_starting_model_in_chain():
    router = ModelRouter()
    # DEFAULT_FALLBACK_CHAIN = ["claude-opus-4-8", "gpt-5.5", "glm-4-flash"]
    chain = router.get_fallback_chain("gpt-5.5")
    assert chain == ["gpt-5.5", "glm-4-flash"]


def test_get_fallback_chain_starting_model_first():
    router = ModelRouter()
    chain = router.get_fallback_chain("claude-opus-4-8")
    assert chain == DEFAULT_FALLBACK_CHAIN
