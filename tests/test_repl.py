...
def test_repl_runs_tasks_and_slash_commands(tmp_path):
    ...
    def command_context_factory(session_id: str) -> CommandContext:
        return CommandContext(
            cwd=str(tmp_path),
            models_config={"default_model": "gpt-5.5", "providers": {}},
    ...
    assert any("status model:gpt-5.5 perm:workspace-write T:off R:- glm:off codex:off claude:off" in item for item in outputs)
